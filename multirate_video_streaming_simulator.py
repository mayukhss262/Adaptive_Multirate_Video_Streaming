...
from network_sim import NetworkSimulator, MODEL_NAMES, VALID_MODELS
...
def _video_fingerprint(video_path: str) -> str:
    """Return a simple fingerprint (size + mtime) for a video file."""
    stat = os.stat(video_path)
    return f"{stat.st_size}_{stat.st_mtime}"


def _load_fingerprint() -> str | None:
    """Load the fingerprint of the video that produced the current chunks."""
    fp_path = os.path.join(config.CHUNKS_DIR, ".video_fingerprint")
...
def _save_fingerprint(video_path: str) -> None:
    """Save the fingerprint of the video that produced the current chunks."""
    os.makedirs(config.CHUNKS_DIR, exist_ok=True)
    fp_path = os.path.join(config.CHUNKS_DIR, ".video_fingerprint")
    with open(fp_path, "w") as f:
        f.write(_video_fingerprint(video_path))


def _clear_chunks() -> None:
    """Remove all existing chunks and encoded files."""
    if os.path.exists(config.CHUNKS_DIR):
        shutil.rmtree(config.CHUNKS_DIR)
    if os.path.exists(config.ENCODED_DIR):
        shutil.rmtree(config.ENCODED_DIR)
    logger.info("Cleared old chunks and encoded files")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Adaptive Multirate Video Streaming Simulator",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Network models (use --model to select):\n"
            + "\n".join(f"  {k:>2d}  {v}" for k, v in sorted(MODEL_NAMES.items()))
        ),
    )
    parser.add_argument(
        "--model", "-m",
        ...
        metavar="MODEL",
        help="Network simulator model (integer 0-6). Default: %(default)s (%(default)s=Sine)",
    )
    parser.add_argument(
        "--input", "-i",
        type=str,
        default=None,
        metavar="VIDEO",
        help="Path to input video file. Default: %(default)s (uses config.INPUT_VIDEO)",
    )
...
def main():
    args = parse_args()
    net_model = args.model
    input_video = args.input if args.input else config.INPUT_VIDEO

    print("=" * 60)
    print("  Adaptive Multirate Video Streaming Simulator")
    print(f"  Network model: {net_model} ({MODEL_NAMES[net_model]})")
    ...
    # Check if existing chunks belong to a different video
    current_fp = _video_fingerprint(input_video)
    stored_fp = _load_fingerprint()
    num_chunks = count_chunks()

    if num_chunks > 0:
        if stored_fp is None or stored_fp != current_fp:
            logger.info("Video changed or no fingerprint — clearing old chunks")
            _clear_chunks()
            num_chunks = 0
    ...
    if num_chunks > 0:
        ...
        encode_and_segment(input_video)
        num_chunks = count_chunks()
        if num_chunks == 0:
            logger.error("Encoding produced no chunks — aborting")
            return
        _save_fingerprint(input_video)

    try:
        duration = get_video_duration(input_video)
        logger.info("Input video: %s (%.1fs, %d chunks)", input_video, duration, num_chunks)
    except Exception:
        duration = num_chunks * config.CHUNK_DURATION
        logger.info("Input video: %s (%d chunks)", input_video, num_chunks)
    ...
    network_sim = NetworkSimulator(model=net_model, use_wall_clock=False)
    abr = ABRController()
    player = Player()
